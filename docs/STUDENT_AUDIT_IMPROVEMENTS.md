...
4. Corrections: lesson-aware links open a draft form. GitHub Issues is enabled on the public repository and is the default destination. The learner reviews and submits; no automatic message is sent, no draft is stored, and no success receipt is claimed before submission. GitHub requires an account and reports are public. An explicitly configured public email uses mailto instead. Privacy text describes this flow. No issue was submitted during tests.
