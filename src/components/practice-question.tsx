...
export function PracticeQuestion({ item, title = "Check your understanding", freshAttempt = false }: { item: PracticeItem; title?: string; freshAttempt?: boolean }) {
  ...
  return <section className="study-panel practice-question" aria-label={title}>
    ...
    <button className="button button-primary" disabled={!ready || choice === null || checked} onClick={() => { if (choice === null) return; recordAnswer(item.id, choice === item.answer, choice, feedbackSeen); recordVisit(); setSubmitted(true); learningEvent({ version: 1, event: "quiz_completed", quiz_id: item.id }); }}>Check answer</button>
