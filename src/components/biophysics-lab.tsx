...
function Curve({ lessonId, label, xLabel, yLabel, maxX, maxY, selectedX, calculate }: { lessonId: string; label: string; xLabel: string; yLabel: string; maxX: number; maxY: number; selectedX: number; calculate: (x: number) => number }) {
  ...
  const source = (references as Record<string,{title:string;url:string}>)[lessonId];
  return <StudyVisual title={label} alt={`${label} Horizontal axis: ${xLabel}. Vertical axis: ${yLabel}. Current selected value: ${calculate(selectedX).toFixed(2)}.`}
    caption="Calculated teaching curve, not measured data. A point and dashed guide mark the selected value; the numerical result and model assumptions are also given in text."
    observe="Predict the direction and size of the change before adjusting the control, then compare the plotted marker with the numerical result."
    credit="Original calculated teaching graphic · Wardhan Medical Study Guide Studios; AI-assisted." sources={[source]}>
    <VisualViewport label="Teaching plot; scroll horizontally on a narrow screen" minWidth={500}><svg className="bp-chart" viewBox="0 0 500 245" role="img" aria-label={label}>
    <title>{label}</title>
  ...
  </svg></VisualViewport></StudyVisual>;
}
...
export function BiophysicsLab() {
  ...
  return <div className="bp-model-grid">
    <section className="study-panel" aria-labelledby="bp-lens-title" id="lenses">
      ...
      <StudyVisual title="Lens and object positions"
        alt={`The object is ${distance} cm left of the converging lens; the focal points are 10 cm either side. The image result is stated above.`}
        caption="Position schematic with a fixed spatial scale. The calculated image can lie outside the drawing; the model neglects aberrations."
        observe="Move the object across the focus and use the numerical result to distinguish real, virtual and parallel-ray cases."
        credit="Original teaching schematic · Wardhan Medical Study Guide Studios; AI-assisted."
        sources={[references["biophysics-image-formation"]]}>
      <VisualViewport label="Teaching plot; scroll horizontally on a narrow screen" minWidth={500}><svg className="bp-chart" viewBox="0 0 500 170" role="img" aria-label={"Lens position schematic: object " + distance + " cm left of lens; focal points 10 cm from lens. Image result is stated above."}>
    ...
    <section className="study-panel" aria-labelledby="bp-attenuation-title" id="attenuation">
      ...
      <Curve lessonId="biophysics-practical-attenuation" label="Exponential attenuation: transmission decreases by half for each half-value layer." xLabel="Half-value layers" yLabel="Transmission (%)" maxX={6} maxY={100} selectedX={layers} calculate={x=>100*transmission(x)} />
    ...
    <section className="study-panel" aria-labelledby="bp-rc-title" id="membrane">
      ...
      <Curve lessonId="biophysics-resting-potential" label="RC charging: voltage approaches its final value exponentially." xLabel="Elapsed time / τ" yLabel="Completed change (%)" maxX={5} maxY={100} selectedX={time} calculate={x=>100*charging(x)} />
    ...
    <section className="study-panel" aria-labelledby="bp-diffusion-title" id="diffusion">
      ...
      <Curve lessonId="biophysics-diffusion-osmosis" label="Diffusion distance grows with the square root of time." xLabel="Time (s)" yLabel="RMS displacement (μm)" maxX={16} maxY={80} selectedX={seconds} calculate={x=>diffusionDistance(200,x)} />
    ...
    <section className="study-panel" aria-labelledby="bp-flow-title" id="flow">
      ...
      <Curve lessonId="biophysics-fluid-flow" label="Flow rises with the fourth power of tube radius at fixed driving pressure." xLabel="Radius / baseline radius" yLabel="Flow / baseline flow" maxX={2} maxY={16} selectedX={radius} calculate={x=>x**4} />
    ...
    <section className="study-panel" aria-labelledby="bp-echo-title" id="ultrasound">
      ...
      <Curve lessonId="biophysics-ultrasound" label="Reflector depth increases linearly with round-trip echo time at fixed sound speed." xLabel="Return time (μs)" yLabel="Depth (cm)" maxX={200} maxY={15.4} selectedX={echo} calculate={x=>echoDepthCm(x)} />
