...
export function ProgressTransfer() {
  ...
  return <section className="study-panel" id="progress-transfer" aria-labelledby="transfer-title">
    ...
    {pending && <div className="study-notice"><h3>Review before importing</h3><p>{Object.keys(pending.progress.answers).length} question histories · {pending.progress.lessons.length} completed lessons · {Object.keys(pending.progress.drafts).length} written notes · {pending.saved.length} saved resources · {Object.values(pending.progress.journey).filter(entry => entry.saved).length} saved summaries recognised.</p><p>We combine completed items and bookmarks. For each question, the newer history wins; equal dates keep this browser’s history. Newer resume and summary changes win. Existing notes and exam plans stay. Activity totals use the larger count, so importing twice does not double them. Unknown or invalid entries are skipped.</p><div className="action-row"><button className="button button-primary" onClick={() => { updateLearning(current => mergeProgress(current, pending.progress)); add(pending.saved); setPending(null); setMessage("Import applied. Your study progress has been updated."); }}>Merge imported progress</button><button className="button button-secondary" onClick={() => { ++request.current; setPending(null); }}>Cancel import</button></div></div>}
