...
export default function StartPage() {
  return <div className="site-container study-page">
    ...
    <section aria-labelledby="choose-subject"><h2 id="choose-subject">Choose your subject</h2><div className="studio-grid">{studySubjects.map(subject => {
      ...
      return <article className="study-panel" key={subject.id}><h3>{subject.title}</h3><p>{studyPaths[subject.id].preparation}</p><p>{subjectLessons(subject.id).length} focused introductions; this is not a complete subject course.</p><ol className="start-sequence">{steps.map(step => <li key={step.href}><Link href={step.href}>{step.title}</Link></li>)}</ol><div className="action-row"><Link className="button button-primary" href={steps[0].href}>Start {subject.title}</Link><Link href={`/study/${subject.id}#coverage`}>See topic coverage</Link></div></article>;
    })}</div></section>
