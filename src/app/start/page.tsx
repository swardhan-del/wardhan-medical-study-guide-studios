...
export default function StartPage() {
  return <div className="site-container study-page">
    ...
    <section aria-labelledby="choose-subject"><h2 id="choose-subject">Choose your subject</h2><div className="studio-grid">{studySubjects.map(subject => {
      const first = subjectLessons(subject.id).find(l => l.id === studyPaths[subject.id].start)!;
      return <article className="study-panel" key={subject.id}><h3>{subject.title}</h3><p>{studyPaths[subject.id].preparation}</p><p>{subjectLessons(subject.id).length} focused introductions</p><div className="action-row"><Link className="button button-primary" href={`/library/${first.id}`}>Start {subject.title}</Link><Link href={`/study/${subject.id}#coverage`}>See topic coverage</Link></div></article>;
    })}</div></section>
