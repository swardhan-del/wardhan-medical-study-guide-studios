...
type Props = { params: Promise<{ topic: string }>; searchParams: Promise<{ topic?: string }> };
...
export default async function AnatomyTopicPage({ params, searchParams }: Props) {
  const { topic } = await params;
  const page = anatomyLearningPages.find((item) => item.slug === topic);
  if (!page) notFound();
  ...
  return (
    <div className="site-container library-page anatomy-learning-page">
      ...
      {topic === "thorax" && <p className="thorax-course-link">Suggested sequence: explore the chest map → explain breathing → answer twelve questions → revisit compartments, pleura and lung relationships.</p>}
      <LearningExplorer key={topic} lessons={page.lessons} initialLessonId={query.topic} />
      <section className="msk-section" id="sources" aria-labelledby="anatomy-sources-title">
        ...
        <p>Public anatomy reference: <a href="https://anatomy.ttuhscep.edu/cardiovascular_system/sup_med_tables.html">Texas Tech: superior mediastinum and thoracic landmarks</a>.</p>
