...
export async function generateMetadata({ params }: Props) { const { subject } = await params; const s = studySubjects.find(s => s.id === subject); return { title: s ? s.title + " · My Study" : "Subject not found", description: s?.description, alternates: { canonical: "/study/" + subject } }; }
export default async function SubjectStudyPage({ params }: Props) {
 ...
 return <div className="site-container study-page">
   <nav className="breadcrumbs" aria-label="Breadcrumb"><Link href="/study">My Study</Link><span aria-hidden="true"> / </span><span aria-current="page">{s.title}</span></nav>
   <header className="study-hero"><p className="eyebrow">My Study · {subject === "physiology" ? "Physiology" : "Subject learning"}</p><h1>{s.title}</h1><p className="interior-lede">{s.description}</p><p>Study a mechanism, explain an answer, then revise it with recap cards, narrated videos or audio where available.</p>
