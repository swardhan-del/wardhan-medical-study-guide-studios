...
import { StudyVisual } from "@/components/study-visual";
...
export default async function RevisionPage({ params }: Props) {
 ...
 const refs = references as Record<string, { title: string; url: string; supportingIds?: string[]; supportingReferences?: { title: string; url: string }[] }>;
 return <article className="site-container revision-page">
  ...
  <PrintableGuide parts={groups.map(g => ({ id: g.id, title: g.number + ". " + g.title,
    content: <><header className="guide-divider"><p className="eyebrow">Part {g.number} · {s.title}</p><h2>{g.title}</h2><p>{g.coverage}</p><p className="muted-note">{g.basis}</p><p>Name: ____________________ Date: ____________________</p><p>After studying: explain the mechanism, sketch the relationships, then check your recall.</p></header>{lessons.filter(l => g.lessonIds.includes(l.id)).map(l => <section key={l.id} className="revision-lesson"><h3>{l.title}</h3>{subject === "anatomy" && <><h4>Objectives</h4><ul>{l.objectives?.map(o => <li key={o}>{o}</li>)}</ul><p><strong>Preparation:</strong> {l.id === "anatomy-foundations" ? "No previous Anatomy lesson is required. Begin with the reference position." : "Anatomical position, planes and directional terms; distinguish arteries, veins, nerves and ducts."} {l.prerequisites?.map(id => <Link key={id} href={"/library/" + id}>{lessons.find(x => x.id === id)?.title}</Link>)}</p></>}{l.steps.map(step => <p key={step.title}><strong>{step.title}.</strong> {step.body}</p>)}{l.workedExample && <div className="revision-worked-example"><h4>{l.workedExample.title}</h4><p>{l.workedExample.prompt}</p><ol>{l.workedExample.solution.map(step => <li key={step}>{step}</li>)}</ol></div>}<TeachingDiagram lessonId={l.id} /><LessonVisuals lessonId={l.id} /><HistologyTopicVisual lessonId={l.id} />{figuresForResource(l.id).map(f => <StudyVisual key={f.id} className="revision-figure" headingLevel={4} title={f.title} alt={f.alt} caption={f.caption} observe={f.observe} credit={`${f.rights}. ${f.modifications}`} sources={[{title:"Image source or supporting scientific reference",url:f.sourceUrl}]}><Image unoptimized src={f.src} width={f.width} height={f.height} alt={f.alt} /></StudyVisual>)}{anatomyRecord(l.id) && <div className="revision-identification"><h4>Draw and identify</h4><p>{anatomyRecord(l.id)!.identification.prompt}</p><div className="drawing-space" aria-label="Space for your labelled drawing" /></div>}<h4>{["histology-foundations-tissues", "anatomy-foundations", ...newEntryLessonIds].includes(l.id) ? "Knowledge Check" : "Check your understanding"}</h4><p>{l.question.prompt}</p><ol type="A">{optionOrder("concept-" + l.id, l.question.options.length).map(i => <li key={i}>{l.question.options[i].text}</li>)}</ol>{applicationQuestions.filter(q => q.topic === l.id).map(q => <div key={q.id}><p>{q.prompt}</p><ol type="A">{optionOrder(q.id, q.options.length).map(i => <li key={i}>{q.options[i].text}</li>)}</ol></div>)}{anatomyRecord(l.id)?.practice.map((q,i) => <div key={q.id}><h4>Practice {i+2}: {q.kind === "Relationship recall" ? "Explain anatomical relationships" : q.kind}</h4><p>{q.prompt}</p></div>)}{l.oralExamination && <><h4>Oral Examination Prompts</h4><ol>{l.oralExamination.map(q => <li key={q.prompt}>{q.prompt}</li>)}</ol></>}{l.summaryChecklist && <><h4>Summary Checklist</h4><ul>{l.summaryChecklist.map(item => <li key={item}>{item}</li>)}</ul></>}<p><strong>Explain aloud:</strong> {l.recall.prompt}</p><p className="revision-source">Source section: {l.section}. Further reading: <a href={refs[l.id].url}>{refs[l.id].title}</a>. {refs[l.id].supportingReferences?.map(ref => <span key={ref.url}><a href={ref.url}>{ref.title}</a>. </span>)}{refs[l.id].supportingIds?.map(id => <span key={id}><a href={refs[id].url}>{refs[id].title}</a>. </span>)}<Link href={"/library/" + l.id}>Full lesson and editorial record</Link>.</p></section>)}</>,
    answers: <>{lessons.filter(l => g.lessonIds.includes(l.id)).map(l => <section key={l.id}><h3>{l.title}</h3><p><strong>{answerLetter("concept-" + l.id, l.question.options.length, l.question.answer)}. {l.question.options[l.question.answer].text}</strong> {l.question.options[l.question.answer].reason}</p>{applicationQuestions.filter(q => q.topic === l.id).map(q => <p key={q.id}><strong>{"heading" in q ? "Knowledge Check" : "Application"}: {answerLetter(q.id, q.options.length, q.answer)}. {q.options[q.answer].text}</strong> {q.options[q.answer].explanation}</p>)}{anatomyRecord(l.id)?.practice.map((q,i) => <div key={q.id}><h4>Practice {i+2}</h4><p>{q.answer}</p></div>)}{l.oralExamination?.map(q => <p key={q.prompt}><strong>{q.prompt}</strong> {q.answer}</p>)}<p><strong>Oral explanation:</strong> {l.recall.answer}</p></section>)}</>
  }))} />
